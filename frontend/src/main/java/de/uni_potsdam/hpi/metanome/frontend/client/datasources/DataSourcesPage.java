 ...
 * connections) and links to actions on them. It also allows to define new data sources.
 *
...
public class DataSourcesPage extends VerticalPanel implements TabContent {
  ...
  public DataSourcesPage(BasePage parent) {
    ...
    temporaryContent.setText(
        "As data sources, you can configure any database connection in the Run Configurations,"
        + "or choose from any CSV-files in the designated folder.");
    this.add(temporaryContent);
  }

  /**
   * This method triggers retrieval of all available data sources and returns a widget with a list
   * of them.
   */
  ...
  private void listCsvFiles() {
    inputDataService.listCsvInputFiles(new AsyncCallback<String[]>() {
      ...
      public void onSuccess(String[] result) {
        ConfigurationSettingDataSource[]
            dataSources =
            new ConfigurationSettingDataSource[result.length];
  ...
  protected void addDataSourcesToList(ConfigurationSettingDataSource[] dataSources,
                                      FlexTable list) {
    int row = 0;
