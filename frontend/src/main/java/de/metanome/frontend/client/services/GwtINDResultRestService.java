...
/**
 * Provides an RestService interface to access GwtINDResultResource methods
 *
 * Created by Alexander Spivak on 04.03.2015.
 */
@Path("/api/gwtINDResults")
public interface GwtINDResultRestService extends RestService {

  @GET
  @Path("/getAll/{executionID}")
  public void listGwtINDResults(@PathParam("executionID") long executionID, MethodCallback<List<GwtINDResult>> callback);

  @GET
  @Path("/getAllFromTo/{executionID}/{sortProperty}/{sortOrder}/{start}/{end}")
  public void listGwtINDResults(@PathParam("executionID") long executionID,
                                @PathParam("sortProperty") String sortProperty,
                                @PathParam("sortOrder") boolean ascending,
                                @PathParam("start") int start,
                                @PathParam("end") int end,
                                MethodCallback<List<GwtINDResult>> callback);
