...
/**
 * Represents an multivalued dependency result with different ranking values.
 */
...
public class MultivaluedDependencyResult implements RankingResult {
  ...
  public MultivaluedDependencyResult(MultivaluedDependency result) {
    this.result = result;
    ...
    if (result.getDependant().getColumnIdentifiers().size() > 0) {
      this.dependantTableName = StringHelper.removeFileEnding(
        result.getDependant().getColumnIdentifiers().iterator().next().getTableIdentifier());
    } else {
  ...
  public void setResult(MultivaluedDependency result) {
    this.result = result;
  }
  ...
  public boolean equals(Object obj) {
    ...
    MultivaluedDependencyResult other = (MultivaluedDependencyResult) obj;
